...
const SliderComponent = React.memo(() => {
  ...
  const handleAddToCollectionClicked = (manga) => {
  ...
  return (
    <Suspense fallback={<SliderComponentSkeleton isDark={isDark} />}>
      <div
        ref={showcaseRef}
        className={`relative w-full ${isDark ? "shadow-[5px_5px_50px_rgba(0,0,0,1)] shadow-black" : ""} min-h-[50vh] sm:h-[60vh] border-b-[16px] ${isDark ? 'border-black' : 'border-white'} overflow-hidden ${isDark ? 'bg-black/60' : 'bg-white/60'}`}
            ...
            <div className="relative h-full mt-3 z-20 flex items-center justify-between">
              <div className="w-[75%] md:w-4/5 px-8 pl-6 md:px-16 md:pl-24 pt-12 pb-32 sm:py-12">
                ...
                <div className="flex flex-wrap gap-3 md:gap-4">
                  <Link href={`/manga/${activeManga?.id}/chapters`} prefetch={true} onClick={() => handleMangaClicked(activeManga)} className={`px-3 md:px-6 py-2 md:py-3 font-medium rounded-sm transition-colors duration-0 text-[11px] sm:text-sm md:text-base inline-block ${isDark ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-800'}`}>
                    Read Now
                  </Link>
                  <button onClick={() => handleAddToCollectionClicked(activeManga)} className={`px-3 md:px-6 py-2 md:py-3  border rounded-sm transition-colors duration-0 text-[11px] sm:text-sm md:text-base ${bookMarks.some((m) => m.manga.id === activeManga.id)?isDark ?"border-white/50 text-white shadow-[inset_0_0_13px_rgba(200,200,200,0.5)] hover:bg-white/10":'border-black text-black shadow-[inset_0_0_13px_rgba(0,0,0,0.5)] hover:bg-black/10':isDark?'border-white text-white hover:bg-white/10':'border-black text-black hover:bg-black/10' }`}>
              ...
              {/* Mobile cover */}
              <Link href={`/manga/${activeManga?.id}/chapters`} prefetch={true} onClick={() => handleMangaClicked(activeManga)} className="absolute top-[40px] right-6 md:right-3 w-[100px] h-40 md:hidden z-30 transition-all duration-500 block" style={{ boxShadow: '0 20px 40px rgba(0,0,0,0.5), 0 0 30px rgba(0,0,0,0.3)' }}>
                <div className={`relative w-full h-full overflow-hidden rounded-sm will-change-transform ${activeManga?.isCoverImageBlurred ? "before:content-[''] before:absolute before:inset-0 before:bg-black/20 before:backdrop-blur-md before:transition-all before:duration-300 hover:before:opacity-0 before:z-10" : ""}`}>
                  <Image width={300} height={300} src={activeManga?.coverImageUrl} alt={activeManga?.title} className="w-full object-cover h-full block" />
              ...
              <div className="hidden md:block md:w-2/5 h-full relative">
                <Link href={`/manga/${activeManga?.id}/chapters`} prefetch={true} onClick={() => handleMangaClicked(activeManga)} className="absolute top-1/2 -translate-y-1/2 right-16 w-64 h-[360px] z-30 transition-all duration-500 block" style={{ boxShadow: '0 20px 40px rgba(0,0,0,0.5), 0 0 30px rgba(0,0,0,0.3)' }}>
                  <div className={`relative w-full h-full overflow-hidden rounded-sm will-change-transform ${activeManga?.isCoverImageBlurred ? "before:content-[''] before:absolute before:inset-0 before:bg-black/20 before:backdrop-blur-md before:transition-all before:duration-300 hover:before:opacity-0 before:z-10" : ""}`}>
                    <Image width={300} height={300} src={activeManga?.coverImageUrl} alt={activeManga?.title} className="w-full h-full object-cover rounded-sm block" />
