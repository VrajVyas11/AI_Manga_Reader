...
const CommentsOnManga = ({ manga, isDark = true }) => {
  ...
  if (loading && comments.length === 0) {
    return (
    <CommentsOnMangaSkeleton isDark={isDark}/>
    );
  }
  return (
    <div className={`${transitionClass} ${isDark ? "" : "bg-white text-black"}`}>
      ...
      >
        ...
        {error && (
          <div
            className={`${isDark ? "mb-6 p-3 sm:p-4 bg-rose-900 border border-rose-700 rounded-2xl text-xs sm:text-sm text-rose-400" : "mb-6 p-3 sm:p-4 bg-rose-100 border border-rose-300 rounded-2xl text-xs sm:text-sm text-rose-700"}`}
          >
            <div className="flex items-center space-x-2 sm:space-x-3">
              <div
                className={`${isDark ? "w-8 h-8 sm:w-10 sm:h-10 bg-rose-800 rounded-full flex items-center justify-center" : "w-8 h-8 sm:w-10 sm:h-10 bg-rose-200 rounded-full flex items-center justify-center"}`}
              >
                <div
                  className={`${isDark ? "w-4 h-4 sm:w-5 sm:h-5 border-2 border-rose-600 rounded-full flex items-center justify-center" : "w-4 h-4 sm:w-5 sm:h-5 border-2 border-rose-400 rounded-full flex items-center justify-center"}`}
                >
                  <div
                    className={`${isDark ? "w-2 h-2 bg-rose-600 rounded-full" : "w-2 h-2 bg-rose-500 rounded-full"}`}
                  ></div>
                </div>
              </div>
              <div>
        ...
        {comments.length === 0 && !loading ? (
          <div
            className={`${isDark ? "text-center py-12 sm:py-16 text-white" : "text-center py-12 sm:py-16 text-gray-900"}`}
          >
            <div className="max-w-sm mx-auto">
              <div
                className={`${isDark ? "w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-4 sm:mb-6 bg-gray-800 rounded-2xl flex items-center justify-center shadow-md" : "w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-4 sm:mb-6 bg-gray-200 rounded-2xl flex items-center justify-center shadow-md"}`}
              >
                <MessageCircle
                  className={`${isDark ? "w-8 h-8 sm:w-12 sm:h-12" : "w-8 h-8 sm:w-12 sm:h-12 text-gray-700"}`}
                />
              </div>
              <h3
              ...
              <p className={`${isDark ? "mb-4 sm:mb-6 text-sm sm:text-base" : "mb-4 sm:mb-6 text-sm sm:text-base"}`}>
                Be the first to share your thoughts about this manga!
              </p>
              <div
                className={`${isDark ? "inline-flex items-center space-x-2 px-4 py-1.5 sm:px-5 sm:py-2 bg-purple-700 rounded-xl text-white font-semibold hover:shadow-lg transition-all duration-0 text-xs sm:text-sm" : "inline-flex items-center space-x-2 px-4 py-1.5 sm:px-5 sm:py-2 bg-gray-300 rounded-xl text-gray-900 font-semibold hover:shadow-lg transition-all duration-0 text-xs sm:text-sm"}`}
              >
                      ...
                      <div className="flex items-start space-x-3 sm:space-x-4 mb-4 sm:mb-5">
                        <div className="relative flex-shrink-0">
                          {comment.avatarUrl ? (
                            <img
                              ...
                              className={`${isDark ? "w-10 h-10 sm:w-12 sm:h-12 rounded-xl object-cover border-2 border-gray-700 transition-all duration-0 shadow-md" : "w-10 h-10 sm:w-12 sm:h-12 rounded-xl object-cover border-2 border-gray-300 transition-all duration-0 shadow-md"}`}
                              loading="lazy"
                              onError={(e) => {
                          ...
                          <div
                            className={`${isDark ? `w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gray-800 flex items-center justify-center border-2 border-gray-700 transition-all duration-0 shadow-md ${comment.avatarUrl ? "hidden" : "flex"
                          ...
                          >
                            <User2
                              className={`${isDark ? "w-5 h-5 sm:w-6 sm:h-6 text-white" : "w-5 h-5 sm:w-6 sm:h-6 text-gray-700"}`}
                            />
                          </div>
                          {/* Online Indicator */}
                          <div
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 sm:space-x-3 mb-1 sm:mb-2">
                            <h3
                              className={`${isDark ? "text-base sm:text-lg font-bold text-gray-100 transition-colors truncate" : "text-base sm:text-lg font-bold text-gray-900 transition-colors truncate"}`}
                            >
                              {comment.username ?? "Anonymous"}
                            </h3>
                            {comment.userTitle && (
                              <span
                                className={`${isDark ? "px-2 py-0.5 bg-purple-700/30 border border-purple-700 rounded-full text-[10px] sm:text-xs text-purple-400 font-medium" : "px-2 py-0.5 bg-gray-200 border border-gray-300 rounded-full text-[10px] sm:text-xs text-gray-700 font-medium"}`}
                              >
                                {comment.userTitle}
                              </span>
                            )}
                        ...
                        {parts.map((part, partIndex) => {
                          ...
                          if (part.type === "normal") {
                            return (
                              <p
                                key={key}
                                className={`${isDark ? "text-gray-400 text-xs sm:text-sm leading-relaxed" : "text-gray-700 text-xs sm:text-sm leading-relaxed"}`}
                              >
                                {part.content}
                              </p>
                            );
                          ...
                          if (part.type === "expandable") {
                            ...
                            return (
                              <div key={key} className="relative">
                                {!isExpanded ? (
                                  <div
                                    className={`${isDark ? "relative cursor-pointer group/expand rounded-xl overflow-hidden" : "relative cursor-pointer group/expand rounded-xl overflow-hidden hover:bg-gray-100"}`}
                                    onClick={() => toggleExpandText(key)}
                                  >
                                    <div
                                      className={`${isDark ? "blur-[2px] select-none p-3 sm:p-4" : "blur-[2px] select-none p-3 sm:p-4"}`}
                                    >
                                      <p
                                        className={`${isDark ? "text-white text-xs sm:text-sm" : "text-gray-700 text-xs sm:text-sm"}`}
                                      >
                                        {part.content}
                                      </p>
                                    </div>
                                    <div
                                      className={`${isDark ? "absolute inset-0 bg-gray-900/90 group-hover/expand:bg-gray-900/80 transition-all duration-0 flex items-center justify-center" : "absolute inset-0 bg-white/90 group-hover/expand:bg-white/80 transition-all duration-0 flex items-center justify-center"}`}
                                    >
                                      <div
                                        className={`${isDark ? "px-4 sm:px-6 py-1.5 sm:py-2 bg-gray-800 flex items-center space-x-2 rounded-xl text-white font-semibold shadow-md transform group-hover/expand:scale-105 transition-all duration-0 text-xs sm:text-sm" : "px-4 sm:px-6 py-1.5 sm:py-2 bg-gray-200 flex items-center space-x-2 rounded-xl text-gray-900 font-semibold shadow-md transform group-hover/expand:scale-105 transition-all duration-0 text-xs sm:text-sm"}`}
                                      >
                                        <Eye
                                          className={isDark ? "w-3 h-3 sm:w-4 sm:h-4" : "w-3 h-3 sm:w-4 sm:h-4"}
                                        />
                                  ...
                                  <div
                                    className={`${isDark ? "space-y-2 sm:space-y-3 p-3 sm:p-4 bg-gray-900 rounded-xl border border-gray-700" : "space-y-2 sm:space-y-3 p-3 sm:p-4 bg-gray-50 rounded-xl border border-gray-300"}`}
                                  >
                                    <p
                                      className={`${isDark ? "text-gray-300 text-xs sm:text-sm leading-relaxed" : "text-gray-700 text-xs sm:text-sm leading-relaxed"}`}
                                    >
                                      {part.content}
                                    </p>
                                    <button
                                      onClick={() => toggleExpandText(key)}
                                      className={`${isDark ? "flex items-center space-x-1 text-white hover:text-gray-300 transition-colors text-xs" : "flex items-center space-x-1 text-gray-700 hover:text-gray-900 transition-colors text-xs"}`}
                                    >
                          ...
                          if (part.type === "spoiler") {
                            ...
                            return (
                              <div
                                key={key}
                                className={`${isDark ? "border border-yellow-700 rounded-xl bg-gray-900 overflow-hidden backdrop-blur-sm" : "border border-yellow-400 rounded-xl bg-yellow-50 overflow-hidden backdrop-blur-sm"}`}
                              >
                                <button
                                  onClick={() => toggleSpoiler(key)}
                                  className={`${isDark ? "w-full p-3 sm:p-4 flex items-center justify-between hover:bg-yellow-900/30 transition-all duration-0 text-sm text-yellow-400" : "w-full p-3 sm:p-4 flex items-center justify-between hover:bg-yellow-200 transition-all duration-0 text-sm text-yellow-700"}`}
                                >
                                  <div className="flex items-center space-x-3">
                                    <div
                                      className={`${isDark ? "w-8 h-8 sm:w-10 sm:h-10 bg-yellow-700 rounded-lg flex items-center justify-center text-xl" : "w-8 h-8 sm:w-10 sm:h-10 bg-yellow-300 rounded-lg flex items-center justify-center text-xl"}`}
                                    >
                                      ⚠️
                                    </div>
                                  ...
                                  <div className="flex items-center space-x-2">
                                    <span
                                      className={`${isDark ? "text-xs px-3 py-1 bg-yellow-700 rounded-full font-medium" : "text-xs px-3 py-1 bg-yellow-300 rounded-full font-medium"}`}
                                    >
                                      {isExpanded ? "Hide spoiler" : "Reveal spoiler"}
                                    </span>
                                    {isExpanded ? (
                                      <ChevronUp
                                        className={`${isDark ? "w-4 h-4 sm:w-5 sm:h-5 text-yellow-500 transition-transform duration-0" : "w-4 h-4 sm:w-5 sm:h-5 text-yellow-700 transition-transform duration-0"}`}
                                      />
                                    ) : (
                                      <ChevronDown
                                        className={`${isDark ? "w-4 h-4 sm:w-5 sm:h-5 text-yellow-500 transition-transform duration-0" : "w-4 h-4 sm:w-5 sm:h-5 text-yellow-700 transition-transform duration-0"}`}
                                ...
                                {isExpanded && (
                                  <div
                                    className={`${isDark ? "px-3 sm:px-4 pb-3 sm:pb-4 border-t border-yellow-700 bg-gray-900 animate-in slide-in-from-top-2 duration-0" : "px-3 sm:px-4 pb-3 sm:pb-4 border-t border-yellow-400 bg-yellow-50 animate-in slide-in-from-top-2 duration-0"}`}
                                  >
                                    <p
                                      className={`${isDark ? "text-gray-300 whitespace-pre-wrap leading-relaxed text-xs sm:text-sm" : "text-yellow-900 whitespace-pre-wrap leading-relaxed text-xs sm:text-sm"}`}
                                    >
                                      {part.content}
                                    </p>
                                  </div>
                ...
                >
                  ...
                  <div className="relative flex items-center justify-center space-x-2 sm:space-x-3">
                    {loadingMore ? (
                      ...
                      <>
                        <ArrowUpCircle
                          className={`${isDark ? "w-4 h-4 sm:w-5 sm:h-5 group-hover:rotate-12 transition-transform duration-0" : "w-4 h-4 sm:w-5 sm:h-5 group-hover:rotate-12 transition-transform duration-0 text-gray-700"}`}
                        />
            ...
            {currentPage >= totalPages && total > 0 && (
              <div className="mt-8 sm:mt-12 text-center px-4 sm:px-6">
                <div className="max-w-lg mx-auto">
                  <h3
                    className={`${isDark ? "text-lg sm:text-xl font-bold text-gray-100 mb-2" : "text-lg sm:text-xl font-bold text-gray-900 mb-2"}`}
                  >
                    {`You've reached the end!`}
                  </h3>

                  <p
                    className={`${isDark ? "text-green-400 mb-4 text-xs sm:text-sm" : "text-green-700 mb-4 text-xs sm:text-sm"}`}
                  >
