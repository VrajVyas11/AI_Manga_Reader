...
const TextToSpeech = ({ text, handleUpload, page, ready, layout = "horizontal", isDark = true }) => {
  ...
  // AGGRESSIVE button handler that stops everything
  // const handleButtonClick = useCallback((e, callback) => {
  //   console.log("TTS Button clicked - stopping everything");
  //   e.stopPropagation();
  //   e.preventDefault();
  ...
  const handleUploadClick = useCallback((e) => {
    console.log("TTS Upload button clicked");
    e.stopPropagation();
    e.preventDefault();
    e.nativeEvent?.stopImmediatePropagation();
  ...
  const handleStopClick = useCallback((e) => {
    ...
    if (!ready) {
      return layout !== "vertical" ? (
        ...
        >
          ...
          />
          <span className="hidden sm:block">Read Aloud</span>
        </button>
      );
    }
    ...
    if (isSpeaking) {
      return (
        <button
          ...
          className={`group py-3 px-1 md:px-2 before:bg-opacity-60 flex items-center justify-start min-w-[36px] sm:min-w-[48px] h-12 sm:h-20 rounded-full cursor-pointer relative overflow-hidden transition-all duration-300 shadow-[0px_0px_10px_rgba(0,0,0,1)] hover:shadow-lg disabled:cursor-not-allowed backdrop-blur-md lg:font-semibold z-10 ease-in-out 
             ${isDark
    ...
    return (
      <div className="tracking-wider flex flex-row justify-center items-center md:gap-2">
        ...
        <div className="relative z-40">
          ...
          {showControls && (
            <div
              className={`tracking-wider h-fit z-50 absolute right-10 md:right-10 -top-16 md:bottom-full mb-2 backdrop-blur-md rounded-lg p-3 sm:p-4 shadow-lg border w-32 sm:w-64 ${isDark
        ...
        {layout !== "vertical" ? (
          ...
          <button
            onClick={handleSpeakClick}
            disabled={isSpeaking}
            className={`tracking-wider font-sans before:bg-opacity-60 sm:min-w-[189px] flex gap-3 justify-start items-center mx-auto text-[12px] sm:text-lg shadow-[0px_0px_16px_rgba(0,0,0,1)] bg-opacity-60 backdrop-blur-md lg:font-semibold isolation-auto before:absolute before:w-full before:transition-all before:duration-700 before:hover:w-full before:-right-full before:hover:right-0 before:rounded-full before:-z-10 before:aspect-square before:hover:scale-200 before:hover:duration-300 relative z-10 px-1 pr-1 sm:px-3 sm:pr-3 py-1 sm:py-2 ease-in-out overflow-hidden border-2 rounded-full group group-hover:border-2 duration-300 transition-transform transform group-hover:rotate-[360deg] ${isDark
          ...
          >
            ...
            />
            <span className="hidden sm:block">Speak</span>
          </button>
        )}
      </div>
    );
  ...
  return <div
    className="tracking-wider flex items-center gap-1 sm:gap-2"
    onClick={(e) => e.stopPropagation()}
  >{renderButton()}</div>;
};
