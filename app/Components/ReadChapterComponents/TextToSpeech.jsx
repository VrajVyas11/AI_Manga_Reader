      ...
      return (
        <button
          ...
          className={`group flex items-center justify-start min-w-[36px] sm:min-w-[48px] h-12 sm:h-20 text-gray-100 rounded-full cursor-pointer relative overflow-hidden transition-all duration-300  
            shadow-[0px_0px_6px_rgba(0,0,0,1)] shadow-red-500 ${layout == "vertical" ? "h-auto p-2" : "h-20 py-2 px-2"
            } bg-red-700 hover:shadow-lg disabled:cursor-not-allowed 
            backdrop-blur-md lg:font-semibold border-gray-50`}
        >
          <Square
            className={`tracking-wider fill-red-500 ${layout == "vertical" ? "w-12 h-12" : "w-12 sm:w-16 h-12 sm:h-16"
    ...
    return (
      <div className="tracking-wider flex flex-row justify-center items-center md:gap-2">
        ...
        {layout !== "vertical" ? (
          ...
          >
            <AudioLines
              className="tracking-wider w-8 h-8 sm:w-12 sm:h-12 group-hover:border-2 group-hover:border-violet-500 transition-all bg-gray-50 backdrop-blur-lg text-violet-900 ease-in-out duration-300 rounded-full border border-gray-700 p-2 sm:p-3 transform group-hover:rotate-[360deg]"
            />
            Speak
          </button>
        )}
